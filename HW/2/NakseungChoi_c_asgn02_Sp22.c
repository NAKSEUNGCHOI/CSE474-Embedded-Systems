...
int mangle(int sid){
    int result, practice;
    result = (sid >> 2) & 0xFFFAF; // 0b11111111111110111111;
    //result = result & ~(1<<7);
    printf("%d ", result);
    if((result & 0x0000A) == 1){
        result = result & 0xFFFF7;
    }else if(result & 0x0000A == 0){
        result = result | 0x00008;
    }
    return result;
    //printf("%d ", result);
}
...
Use these to test your code.  Do not modify the code below.  See sample output file.
*************************************************************************/
